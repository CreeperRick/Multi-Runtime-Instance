...
# ── FIX: use filename only, not full path, so cwd= is the source of truth ────
...
def run_code(instance_id):
    ...
    if runtime == 'python':
        ...
        # ✅ FIX: just "main.py" — cwd handles the path, no doubling
    ...
    try:
        result = subprocess.run(
            command,
            cwd=str(instance_path),   # working directory = instance folder
            capture_output=True,
            text=True,
            timeout=timeout,
        )
        ...
        if settings.get('auto_save_logs', True):
            log_execution(instance_id, runtime, result.stdout, result.stderr,
